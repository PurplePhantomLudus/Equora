...
function setPlayerCount(num) {
    ...
    container.innerHTML = "";
    for(let i=1; i<=num; i++) {
        container.innerHTML += `<input type="text" id="name-p${i}" placeholder="Nombre del Jugador ${i}" class="w-full p-4 rounded-2xl bg-slate-100 font-black border-2 border-transparent focus:border-emerald-500 outline-none text-slate-700">`;
    }
...
function calculatePoints() {
    ...
    const base = (Number(document.getElementById('t1').value)||0)*1 + t2*3 + t3*7 + m2*3 + m3*7 + (Number(document.getElementById('fld').value)||0)*5 + (Number(document.getElementById('bld').value)||0)*5;
    ...
    return { total: base + water + fauna + spiritBono };
}
...
function showFinalRanking() {
    ...
    let html = `<h3 class="text-2xl font-black italic uppercase text-slate-800 mb-6">🏆 Balance de Victoria</h3>`;
    allScores.forEach((s, i) => {
        ...
        html += `<div class="flex justify-between items-center bg-slate-800 p-5 rounded-3xl text-white shadow-lg mb-3 border border-slate-700">
                    ...
                    <div class="text-right text-emerald-400 font-black text-3xl">${s.points}<small class="text-xs ml-1">pts</small></div>
                 </div>`;
    });

    if(totalPlayers === 1) {
        const p = allScores; // CORRECCIÓN: Acceso correcto al primer elemento
        const table = { 40:1, 70:2, 90:3, 110:4, 130:5, 140:6, 150:7, 160:8 };
        let count = 0;
        Object.keys(table).forEach(pts => { if(p.points >= Number(pts)) count = table[pts]; });
...
window.onload = () => {
    ...
    for(let i=1; i<=9; i++) {
        grid.innerHTML += `
            <div class="bg-white rounded-2xl p-2 text-center shadow-sm border border-emerald-100">
                ...
                <input type="number" placeholder="Pts" class="fauna-input w-full p-1 text-center text-sm font-black bg-emerald-50 rounded-lg outline-none">
            </div>`;
    }
};